...
import io.flexio.services.api.documentation.handlers.CreateClassifer;
...
public class VersionExtractor implements Comparable<VersionExtractor> {
    ...
    public void parse() throws VersionNotRecognizedException {
        ...
        if (m.matches()) {
            ...
            if (m.group(4) == null) {
                this.isSnapshoot = false;
            } else {
                this.isSnapshoot = m.group(4).toUpperCase().contains("SNAPSHOOT");
                this.isSnapshoot |= m.group(4).toUpperCase().contains("DEV");
            }
